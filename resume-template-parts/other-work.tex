%-------------------------------------------------------------------------------
%	SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Other Work}


%-------------------------------------------------------------------------------
%	SUBSECTION TITLE
%-------------------------------------------------------------------------------
\cvsubsection{Open Source}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {WordPress Contributor} % Role/Title
    {WordPress / Gutenberg} % Organization
    {} % Location
    {2014 -- Present} % Date(s)
    {
      \begin{cvitems}
        \item {Contributions in \href{https://github.com/WordPress/gutenberg/pulls?q=is\%3Apr+author\%3Adesaiuditd}{\color{awesome}Gutenberg}}
        \item {\href{https://core.trac.wordpress.org/report/25?USER=desaiuditd}{\color{awesome}Contributions} in \href{https://github.com/WordPress/wordpress-develop/pulls?q=is\%3Apr+author\%3Adesaiuditd}{\color{awesome}WordPress Core}}
        \item {Other \href{https://profiles.wordpress.org/desaiuditd/\#content-plugins}{\color{awesome}WordPress Plugins}}
      \end{cvitems}
    }

%---------------------------------------------------------
\end{cventries}


%-------------------------------------------------------------------------------
%	SUBSECTION TITLE
%-------------------------------------------------------------------------------
\cvsubsection{Freelance Work}


%-------------------------------------------------------------------------------
%	CONTENT
%-------------------------------------------------------------------------------
\begin{cventries}

%---------------------------------------------------------
  \cventry
    {Web Engineer} % Role/Title
    {10up} % Organization
    {Remote / EMEA (UK)} % Location
    {Jun 2022 -- Sep 2023} % Date(s)
    {}

%---------------------------------------------------------
  \cventry
    {Lead Engineer} % Role/Title
    {Moxie Media Group} % Organization
    {Remote / USA} % Location
    {Dec 2014 -- Aug 2015} % Date(s)
    {}

%---------------------------------------------------------
  \cventry
    {WordPress Engineer} % Role/Title
    {XWP.co Pty. Ltd.} % Organization
    {Remote / Australia} % Location
    {Feb 2014 -- Aug 2015} % Date(s)
    {}

%---------------------------------------------------------
\end{cventries}
